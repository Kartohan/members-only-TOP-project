...
exports.form_member_post = [
  body("number")
    .trim()
    .isLength({ min: 1 })
    .withMessage("Enter a number")
    .custom((value, { req }) => {
      if (value === req.session.answer) {
        return true;
      }
      throw new Error("Answer in not correct");
    })
    .escape(),
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      res.render("membership", {
        user: req.user,
        numberFrom: req.body.numberFrom,
        errors: errors.array({ onlyFirstError: true }),
      });
    } else {
      User.findByIdAndUpdate(req.user.id, { member: true }, {}, (err) => {
        if (err) {
          return next(err);
        }
        req.flash(
          "status",
          "Congratulations! Now you are a member! Let's write a message!"
        );
        res.redirect("/");
      });
    }
...
function secret() {
  ...
  let array = pi.split(" ");
  array = array.slice(1);
  let x = Math.round(Math.random() * array.length - 10);
  let newarray = [];
  for (let index = x; index < x + 10; index++) {
